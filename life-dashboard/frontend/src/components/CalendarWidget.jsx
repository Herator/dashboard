...
import { monthGrid, toYMD } from "../dateUtils";
...
export default function CalendarWidget() {
  ...
  useEffect(() => {
    ...
    getExternalEvents(toYMD(first), toYMD(last))
      .then((data) => {
        setExternalEvents(data.events);
        setFeedErrors(data.errors || []);
      })
      .catch((err) => setError(err.message));
    // `weeks` is derived from `monthDate` each render; comparing the dates
    // directly would refetch every render, so key off the month instead.
  ...
  async function handleAddFeed(e) {
    ...
    try {
      ...
      const first = weeks[0][0];
      const last = weeks[weeks.length - 1][6];
      const data = await getExternalEvents(toYMD(first), toYMD(last));
      setExternalEvents(data.events);
      setFeedErrors(data.errors || []);
    } catch (err) {
      setError(err.message);
            ...
            >
              ...
              <span className="calendar-day-dots">
                {dayItems.slice(0, 4).map((item, i) => (
                  <span key={i} className="calendar-dot" style={{ "--accent": item.color }} />
                ))}
              </span>
            </button>
          );
